...
import {
    AddPatientReducer_Types,
    IAddPatientError,
} from '../Reducers/AddPatientReducer';
...
function SetAddPatientCustomerProfileAction(
...
function SetAddPatientIsCheckingForCustomerAction(
    isCheckingForCustomer: boolean,
) {
    return {
...
export const SetAddPatientPhoneNumber = (phoneNumber: string): Action =>
    SetAddPatientPhoneNumberAction(phoneNumber);

export const SetAddPatientCustomerProfile = (
    customerProfile: ICustomerProfileOutgoing,
): Action => SetAddPatientCustomerProfileAction(customerProfile);

export const SetAddPatientIsCheckingForCustomer = (
    isCheckingForCustomer: boolean,
): Action => SetAddPatientIsCheckingForCustomerAction(isCheckingForCustomer);

export const SetAddPatientIsCustomerExists = (
    isCustomerExists: boolean,
): Action => SetAddPatientIsCustomerAction(isCustomerExists);

export const SetAddPatientIsMakingDoneCall = (
    isMakingDoneCall: boolean,
): Action => SetAddPatientIsMakingDoneCallAction(isMakingDoneCall);

export const SetAddPatientIsDoneCallSuccess = (
    isDoneCallSuccess: boolean,
): Action => SetAddPatientIsDoneCallSuccessAction(isDoneCallSuccess);

export const SetAddPatientIsInvalidNumber = (
    isInvalidNumber: boolean,
): Action => SetAddPatientIsInvalidNumberAction(isInvalidNumber);

export const ClearAddPatientState = (): Action => ClearPatientStateAction();

export const SetAddPatientPhoneNumberValidationError =
    (errorMessage: string): ThunkAction<void, RootState, null, Action> =>
    async (dispatch, getState) => {
        const addPatientStateErrors =
            getState().AddPatientState.validationErrors;

        addPatientStateErrors.phoneNumber = errorMessage;
        dispatch(SetAddPatientErrorsAction(addPatientStateErrors));
    };

export const SetAddPatientNameValidationError =
    (errorMessage: string): ThunkAction<void, RootState, null, Action> =>
    async (dispatch, getState) => {
        const addPatientStateErrors =
            getState().AddPatientState.validationErrors;
...
export const SetAddPatientAgeValidationError =
    (errorMessage: string): ThunkAction<void, RootState, null, Action> =>
    async (dispatch, getState) => {
        const addPatientStateErrors =
            getState().AddPatientState.validationErrors;
...
export const ClearAddPatientValidationErrors = (): Action =>
    SetAddPatientErrorsAction({} as IAddPatientError);
