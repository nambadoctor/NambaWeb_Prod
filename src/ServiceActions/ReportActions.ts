...
export const GetReports =
    ...
    async (dispatch, getState) => {
        ...
        try {
            ...
            dispatch(
                SetNonFatalError('Could not get reports for this appointment'),
            );
        }
    };
...
export const GetAllReportsForCustomer =
    (
        ...
        customerId: string,
        currentReports: IReportIncomingData[] | null,
    ): ThunkAction<void, RootState, null, Action> =>
    async (dispatch, getState) => {
        try {
            let response = await getCall(
                {} as Array<IReportIncomingData>,
                GetCustomerAllReportsEndPoint(organisationId, customerId),
                'GetReports',
            );

            if (response) {
                var filterReports = FilterAllAndCurrentReports(
                    currentReports,
                    response.data,
                );
